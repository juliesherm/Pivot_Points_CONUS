...
gs_dates_median_tile_path = base_path+'results/'+park+'/gs_dates/'+win+'/tiled/'
tile_path_out = base_path+'results/'+park+'/gs_summary/'+win+'/'+VI+'/tiled/'
...
for yr in range(2000,2024):
    ...
    for t in range(0,len(vi_tiles)):
        st = time.time()
        #Use the first file as a template to read metadata
        with rasterio.open(vi_tiles[t]) as src:
            meta = src.meta
        meta.update({"dtype": 'float64', "count":1})
        #gs_start_dims = gs_date_tiles[t].split("_")[-2:]
        tile_start_dims = vi_tiles[t].split("_")[-2:]
        rely_start_dims = rely_tiles[t].split("_")[-2:]
        ...
        #Open VI data
        vi_rast = rioxarray.open_rasterio(vi_tiles[t])
        rely_rast = rioxarray.open_rasterio(rely_tiles[t])#, chunks = {'band':-1, 'x':'auto', 'y': 'auto'})
